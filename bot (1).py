...
امکانات:
  ...
  - ثبت خودکار مشخصات و تعداد پیام هر عضو در پایگاه‌داده MongoDB Atlas (دائمی)
...
MONGO_DB_NAME = os.environ.get("MONGO_DB_NAME", "mobasser_bot")
...
def init_db():
    """اتصال به MongoDB و آماده‌سازی ایندکس‌ها."""
    global mongo_client, members_col
    if not MONGO_URI:
        raise RuntimeError(
            "متغیر محیطی MONGO_URI تنظیم نشده! رشته‌ی اتصال MongoDB Atlas را وارد کن."
        )
...
async def ensure_indexes():
    await members_col.create_index([("chat_id", 1), ("user_id", 1)], unique=True)


async def upsert_member(chat_id: int, user_id: int, username: str, full_name: str):
...
async def get_member_row(chat_id: int, user_id: int):
    return await members_col.find_one({"chat_id": chat_id, "user_id": user_id})
...
async def remove_one_warning(chat_id: int, user_id: int) -> int:
    doc = await members_col.find_one({"chat_id": chat_id, "user_id": user_id})
    current = doc["warnings"] if doc else 0
    new_value = max(current - 1, 0)
    await members_col.update_one(
        {"chat_id": chat_id, "user_id": user_id},
        {"$set": {"warnings": new_value}},
        upsert=True,
    )
    return new_value
...
async def track_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await upsert_member(chat.id, user.id, username, full_name)
...
async def warn_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await upsert_member(
        chat_id, target.id, f"@{target.username}" if target.username else "-", target.full_name
    )
    reason = " ".join(context.args) if context.args else "بدون دلیل ذکرشده"
    warnings = await add_warning(chat_id, target.id)
...
async def unwarn_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target is None:
        ...
        return
    chat_id = update.effective_chat.id
    warnings = await remove_one_warning(chat_id, target.id)
    await update.effective_message.reply_text(
...
async def warnings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target is None:
        ...
        return
    chat_id = update.effective_chat.id
    row = await get_member_row(chat_id, target.id)
...
async def info_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"تعداد پیام‌ها: {row.get('message_count', 0)}\n"
        f"تعداد اخطار: {row.get('warnings', 0)}/{MAX_WARNINGS}"
    )
...
async def post_init(application: Application):
    """بعد از راه‌اندازی اپلیکیشن، ایندکس‌های دیتابیس ساخته می‌شن."""
    await ensure_indexes()
    logger.info("اتصال به MongoDB برقرار و ایندکس‌ها آماده شد.")
